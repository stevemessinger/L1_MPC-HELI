function xdot=helicopter(t, x, u, heli, w)
%{
...
heli.S_fus_z = 0.15; % Vertical fuselage drag area (m^2)
...
xdot=nan(18,1); % allocate an array for xdot
...
S_fus_z = heli.S_fus_z; % Vertical fuselage drag area (m^2)
...
%fuselage forces 
V_imr = lambda_0*V_tip_mr; %NOT SURE IF RIGHT
V_inf = sqrt(u_a^2 + v_a^2 + (w_a + V_imr)^2);
X_fus = -0.5*rho*S_fus_x*u_a*V_inf;
Y_fus = -0.5*rho*S_fus_y*v_a*V_inf;
Z_fus = -0.5*rho*S_fus_z*(w_a + V_imr)*V_inf;
...
while diff > 0.001
    ...
    % store previous lambda value
    old_lambda_tr = lambda_0_tr; 
    % calculate new lambda value 
    G = (mu_tr^2) + (lambda_0_tr - mu_z_tr)^2; 
    h_j = -((2*n_w*lambda_0*sqrt(G) - C_T)*G)/(2*n_w*(G^(3/2)) + ((a_tr*sigma_tr)/4)*G - C_T_tr*(mu_z_tr - lambda_0_tr)); 
    lambda_0_tr = lambda_0_tr + f_j*h_j; 
end
...
Y_vf = min(max(Y_vf,(-0.5*rho*S_vf*(V_inf_tr^2 + v_vf^2))),(0.5*rho*S_vf*(V_inf_tr^2 + v_vf^2)));
N_vf = -Y_vf*l_tr; 
L_vf = Y_vf*h_tr; 
...
xdot(6)= u*q - v*p + g*cos(euler(3))*cos(euler(2)) + (Z_mr + Z_fus + Z_ht)/m;
% angular accelerations 
xdot(11)= q*r*(I_yy - I_zz)/I_xx + (L_mr + L_vf + L_tr)/I_xx; 
